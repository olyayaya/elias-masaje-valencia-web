...
describe("gallery thumbnail crop migration", () => {
  it("keeps exactly one canonical crop migration file", () => {
    const files = cropMigrationFiles();
    expect(files).toEqual(["20260818063851_f3c96eb0-7f8a-4fdb-b4db-7f71fdf5ad18.sql"]);
  });

  it("declares 50/50/1 defaults and three CHECK range constraints", () => {
    const [file] = cropMigrationFiles();
    ...
    expect(sql).toMatch(/gallery_items_thumbnail_zoom_range CHECK \(thumbnail_zoom >= 1 AND thumbnail_zoom <= 3\)/i);
