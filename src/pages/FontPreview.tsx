...
const FontPreview = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      {/* Dynamic style override scoped to preview zone */}
      <style>{`
        .font-preview-zone .fp-display {
          font-family: ${pair.display} !important;
        }
        .font-preview-zone .fp-body {
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        ...
        {/* Preview zone */}
        <div className="font-preview-zone" key={pair.id}>
          {/* Font info */}
          <div className="flex gap-8 text-sm text-muted-foreground fp-body mb-12">
            <div>
          ...
          <div className="bg-card rounded-2xl p-10 md:p-14 mb-12">
            <p className="text-xs tracking-[0.3em] uppercase mb-4 text-muted-foreground fp-body">
              Valencia · Massage · Wellness
            </p>
            <div className="text-4xl md:text-5xl leading-[1.15] mb-5 fp-display font-medium">
              {SAMPLE.headline}
            </div>
            <p className="text-lg text-muted-foreground leading-relaxed max-w-lg mb-8 fp-body">
              {SAMPLE.subhead}
            </p>
            <span className="inline-block bg-foreground text-background px-8 py-3 rounded-full text-sm fp-body">
              Book Now
            </span>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-12">
            <div>
              <div className="text-2xl mb-4 fp-display font-medium">
                English Body Text
              </div>
              <p className="text-base text-muted-foreground leading-[1.85] fp-body">
                {SAMPLE.body}
              </p>
            </div>
          ...
          <div className="border-t border-border pt-8 space-y-0 divide-y divide-border">
            {["Deep Tissue Massage", "Relaxation Therapy", "Hot Stone Treatment"].map((s) => (
              <div key={s} className="flex items-center justify-between py-6">
                <div>
                  <div className="text-xl mb-1 fp-display font-medium">
                    {s}
                  </div>
                  <p className="text-sm text-muted-foreground fp-body">
                    60 min · €55
                  </p>
                </div>
                <span className="text-sm border border-foreground/20 px-5 py-2 rounded-full fp-body">
                  Reserve
                </span>
              </div>
            ))}
