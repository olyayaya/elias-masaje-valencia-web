...
CREATE TABLE IF NOT EXISTS public.reviews (
  ...
  CONSTRAINT reviews_dedupe_key_key UNIQUE (dedupe_key)
);
...
REVOKE EXECUTE ON FUNCTION public.review_is_public(integer, boolean) FROM PUBLIC;
...
GRANT SELECT (id, singleton, section_enabled, allowed_ratings, sort_mode, updated_at)
...
CREATE POLICY "Public reviews are readable"
  ...
  USING (public.review_is_public(rating, visible));
